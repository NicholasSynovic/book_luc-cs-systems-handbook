...
#. generate a private and public key pair (Key Type: RSA Key Length:
   1024);
...
#. Generate a private and public key pair:
   ...
   This passphrase is not sent to the remote host, and it is only used
   to protect your private key. Otherwise, anyone who has access to your
...
#. Generate a private and public key pair
   ...
   -  Double click PuTTYgen.exe, press [Generate] button, keep moving
      mouse. Once the keys are generated, enter a passphrase for your
      private key, or leave the boxes empty if you do not want to
      protect your private key with a passphrase.

   -  This passphrase is not sent to the remote host, and it is only
      used to protect your private key. Otherwise, anyone who has access
      to your private key can authenticate to your account
      automatically.
...
#. Store Private key in PuTTY SSH authentication agent
   ...
   -  Start the PuTTY SSH authentication agent PAGEANT.EXE (double click
      the icon).

   -  After it started, right-click its syspanel icon (in the
      right-bottom corner of your screen)and select "add key". Select
      your private key file, say C:ppp.ppk, and type in your passphrase.
...
#. Generate a private and public key pair
   ...
   -  In the toolbar of ssh client, there are several icons. First from
      left is "Save", and second is "Print". The third from RIGHT, is
      "Settings".
   ...
   -  Under "Key pair management", click "Generate New", then click
      Next.
...
#. Store private key
   ...
   -  In the "File Name:" field, enter a name for the file where SSH
      ...
      stored in a file with the same name, plus a .pub extension.

   -  In the boxes next to "Passphrase:", enter a passphrase for your
      private key, or leave the boxes empty if you do not want to
      protect your private key with a passphrase.
...
#. Save Public key on the Linux server
   ...
   -  Under "Public Key management", click "View". Notepad or another
      editor will open xxx.pub You may save it in a file. Then, you need
      to append that file to ~/.ssh/authorized_keys on the Linux server.
...
#. Use ssh client to connect to the Linux server
   ...
   -  You will be prompted for the passphrase for your private key (if
      ...
      private key, you will not receive a request for a passphrase when
      connecting to the remote host.

#. See http://kb.iu.edu/data/amzx.html for additional details. We have
   adapted our instructions from this page.
...
#. Download Source
   ...
   -  Please choose [Install from Internet] option, setup.exe creates a
      local directory to store the packages before actually installing
      the contents. The Root Directory for Cygwin (default C:cygwin)
      will become / within your Cygwin installation.
...
#. After installing Cygwin/openssh, you can configure OpenSSH for Public
   Key Authentication
